...
export function ModelBadge({ model }: { model: string }) {
  ...
  return (
    <span
      className={cn(
        "rounded-md px-1.5 py-0.5 font-mono text-[9.5px] font-semibold uppercase tracking-wider",
        isFlash ? "bg-accent text-accent-foreground" : "bg-muted text-muted-foreground"
      )}
      title={model}
    >
      {isFlash ? "Gemini" : model.split("-")[0]}
    </span>
  );
}
